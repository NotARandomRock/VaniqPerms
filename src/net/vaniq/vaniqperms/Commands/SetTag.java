        ...
        implements CommandExecutor
{
    public boolean onCommand(CommandSender sender, Command cmd, String lebel, String[] args) {
        ...
        if (sender.hasPermission("vaniq.settag")) {

            if (args.length == 0) {
                p.sendMessage("§cUsage: /settag (Rank) (Prefix)");
                return true;
            }
            ...
            try {
                ...
                results.next();
                    ...
                    PreparedStatement statement = Main.plugin.getConnection().prepareStatement("UPDATE " + Main.tablerank + " SET tag=? WHERE RANKS=?");
                    statement.setString(1, prefix.toString().trim());
                    statement.setString(2, String.valueOf(rank).toUpperCase());
                    statement.executeUpdate();

                    p.sendMessage("§7You have set the rank §d" + rank.toUpperCase() + " §7tag to §f" + ChatColor.translateAlternateColorCodes('&', prefix.toString().trim()));
